...
def matches(h:dict,size:int)->bool:
    return all([
      ...
      is_ptr(h["connections_ptr"],size,4),
      h["music"]==MUSIC_SEVII_ROUTE,h["mapsec"]==MAPSEC_SEVII_ISLE_7,
...
def discover(data:bytes)->list[dict]:
    fixed=bytes([MAPSEC_SEVII_ISLE_7,0,WEATHER_SUNNY,MAP_TYPE_ROUTE,1,HEADER_FLAGS,0,0])
    out=[];start=0
    while True:
      ...
      if not (l["width"]==1 and l["height"]==1 and l["border_width"]==2 and l["border_height"]==2): continue
...
def analyze_rom(data:bytes)->dict:
    c=discover(data)
    return {
      "slot":"MAP_PROTOTYPE_SEVII_ISLE_7","map_group":3,"map_num":51,
      "candidate_count":len(c),"candidates":c,"safe_to_repoint":len(c)==1
    }
...
def main():
    ...
    if r["candidate_count"]==0: print("RC_DEPLOY_DISTRICT_SLOT=NOT_FOUND"); return 2
    if not r["safe_to_repoint"]: print("RC_DEPLOY_DISTRICT_SLOT=AMBIGUOUS"); return 3
    print(f"RC_DEPLOY_DISTRICT_SLOT=READY:0x{r['candidates'][0]['offset']:X}"); return 0
