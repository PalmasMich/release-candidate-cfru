...
def build_payloads(data:bytes)->dict:
    ...
    required=hub_probe["size"]+3+marina_probe["size"]
    ...
    allocation_end=marina_file_offset+marina["size"]
...
def patch_rom(source:Path,output:Path)->Path:
    ...
    print("RC_CUSTOM_MAPS=DELIVERY_HUB+MARINA")
...
def main():
    p=argparse.ArgumentParser(description="Atomically install Release Candidate Delivery Hub and Marina custom maps.")
