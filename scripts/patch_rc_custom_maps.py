...
def build_payloads(data:bytes)->dict:
    ...
    required=hub_probe["size"]+3+marina_probe["size"]+3+port_probe["size"]+3+castello_probe["size"]
    ...
    allocation_end=castello_file_offset+castello["size"]
...
def patch_rom(source:Path,output:Path)->Path:
    ...
    print("RC_CUSTOM_MAPS=DELIVERY_HUB+MARINA+PORT_LINK+CASTELLO")
...
def main():
    p=argparse.ArgumentParser(description="Atomically install Release Candidate Delivery Hub, Marina, Port Link and Castello custom maps.")
