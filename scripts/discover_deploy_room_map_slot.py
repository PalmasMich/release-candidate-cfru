...
def matches(h:dict,size:int)->bool:
    return all([
      ...
      h["connections_ptr"]==0,
      h["music"]==MUSIC_ROUTE3,h["mapsec"]==MAPSEC_ROUTE6,h["cave"]==0,
      h["weather"]==WEATHER_NONE,h["map_type"]==MAP_TYPE_INDOOR,h["biking"]==0,
      h["flags"]==0,h["floor"]==0,h["battle"]==BATTLE_SCENE_NORMAL
    ])

def discover(data:bytes)->list[dict]:
    fixed=bytes([MAPSEC_ROUTE6,0,WEATHER_NONE,MAP_TYPE_INDOOR,0,0,0,BATTLE_SCENE_NORMAL])
    out=[];start=0
    while True:
      ...
      if int.from_bytes(data[o+0x10:o+0x12],"little")!=MUSIC_ROUTE3: continue
      try:h=parse_header(data,o)
      except ValueError:continue
      if not matches(h,len(data)): continue
      try:l=parse_layout(data,h["layout_ptr"])
      except ValueError:continue
      if not (l["width"]==11 and l["height"]==9 and l["border_width"]==2 and l["border_height"]==2): continue
...
def analyze_rom(data:bytes)->dict:
    c=discover(data)
    return {"slot":"MAP_ROUTE6_UNUSED_HOUSE","map_group":18,"map_num":1,"candidate_count":len(c),"candidates":c,"safe_to_repoint":len(c)==1}
...
def main():
    ...
    if r["candidate_count"]==0: print("RC_DEPLOY_ROOM_SLOT=NOT_FOUND"); return 2
    if not r["safe_to_repoint"]: print("RC_DEPLOY_ROOM_SLOT=AMBIGUOUS"); return 3
    print(f"RC_DEPLOY_ROOM_SLOT=READY:0x{r['candidates'][0]['offset']:X}"); return 0
