...
def matches(h:dict,size:int)->bool:
    return all([
      ...
      h["music"]==MUSIC_SEVII_ROUTE,h["mapsec"]==MAPSEC_SEVII_ISLE_6,
...
def discover(data:bytes)->list[dict]:
    fixed=bytes([MAPSEC_SEVII_ISLE_6,0,WEATHER_SUNNY,MAP_TYPE_ROUTE,1,HEADER_FLAGS,0,0])
    out=[];start=0
    while True:
...
def analyze_rom(data:bytes)->dict:
    c=discover(data)
    return {
      "slot":"MAP_PROTOTYPE_SEVII_ISLE_6","map_group":3,"map_num":50,
...
def main():
    ...
    if r["candidate_count"]==0: print("RC_CASTELLO_SLOT=NOT_FOUND"); return 2
    if not r["safe_to_repoint"]: print("RC_CASTELLO_SLOT=AMBIGUOUS"); return 3
    print(f"RC_CASTELLO_SLOT=READY:0x{r['candidates'][0]['offset']:X}"); return 0
