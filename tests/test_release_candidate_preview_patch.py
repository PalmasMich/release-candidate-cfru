...
class ReleaseCandidatePreviewPatchTest(unittest.TestCase):
    ...
    def test_thick_preview_exposes_cagliari_corporate_identity(self):
        p=load_patcher(); patched=build_patched_fixture(p)
        for text in ("Welcome to Release Candidate!","YOUR ID?","TEAMMATE ID?","LEAD: Hey! Wait!\nNo field test yet!","LEAD: Scope changed!\nField checks start today!","You need a resource for\nthe Port Link test.","Come on!\nDelivery Hub, now!","CAGLIARI\nFirst sprint starts here!","Delivery is incredible!","DELIVERY HUB - CAGLIARI","Three resources are ready.","The build candidates are inside\nthese three team slots.","Back then, every release had\none clean requirement.","Now scope changes daily.\nThese three passed intake.","Inside these slots are three\nrelease candidates.","I don't chase metrics.\nI just happen to lead them.","Let's benchmark our resources!","KPI check: show velocity!","KPI is GREEN!","See you at stand-up!","LEAD: {PLAYER}, validate your\nresource with field tests.","Evidence beats a status report.","PORT LINK\nCAGLIARI - MARINA PORTO","MISTRILLO is in this grass.\nField test it now.","See those shortcuts on Port Link?","MARINA PORTO \nDEPLOY BLOCKED - CHECK SCOPE","That build at your side!\nYou're on the release, right?","Marina Porto is live.\nKeep the package stable.","Hey, stakeholder!\nScope is still open!","Sorry. The sign-off\nis still pending.","This deploy gate is still closed.","Stakeholder approval pending.","This scope is out of bounds!","Time is budget. Keep meetings\nshort and useful.","MARINA PORTO DEPLOY GATE","DEPLOY GATE locked: scope open."): self.assertIn(p.encode_text(text),patched)

    def test_required_identity_gate_covers_preview_pillars(self):
        p=load_patcher(); expected={p.encode_text(x) for x in ("Welcome to Release Candidate!","LEAD: Scope changed!\nField checks start today!","CAGLIARI\nFirst sprint starts here!","DELIVERY HUB - CAGLIARI","Three resources are ready.","Inside these slots are three\nrelease candidates.","Let's benchmark our resources!","KPI check: show velocity!","LEAD: {PLAYER}, validate your\nresource with field tests.","PORT LINK\nCAGLIARI - MARINA PORTO","MISTRILLO is in this grass.\nField test it now.","MARINA PORTO \nDEPLOY BLOCKED - CHECK SCOPE","Marina Porto is live.\nKeep the package stable.","MARINA PORTO DEPLOY GATE")}; self.assertEqual(set(p.REQUIRED_VISIBLE_TEXTS),expected)
