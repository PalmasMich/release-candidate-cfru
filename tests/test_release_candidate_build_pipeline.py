...
class ReleaseCandidateBuildPipelineTest(unittest.TestCase):
    ...
    def test_pipeline_order(self):
        self.assertEqual(load_builder().PIPELINE, ("CHAPTER1_PREFLIGHT", "DPE", "RC_STARTER_RUNTIME", "CFRU", "RC_PREVIEW_PATCH", "RC_OPENING_AUDIT", "PORT_LINK_DISCOVERY", "PORT_LINK_TRAINER_PATCH", "DELIVERY_HUB_MAP_PLAN", "RC_CUSTOM_MAPS_PATCH"))
    ...
    def _run_fake_pipeline(self, tmp, *, fail_cfru=False, discovery=2, trainer=1, hub=0, custom=0):
    ...
    def test_preflight_runs_before_dpe_build(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            builder.run_pipeline(
                ...
                prepare_delivery_hub_map=lambda _p: 0,
                apply_custom_maps=lambda source, destination: (
                    destination.write_bytes(source.read_bytes() + b"-maps"), 0
                )[1],
            )
    ...
    def test_pipeline_applies_port_link_trainer_when_ready(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(output.read_bytes(), b"dpe-plus-cfru-preview-trainer-maps")
    ...
    def test_pipeline_blocks_when_custom_maps_fail(self):
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(RuntimeError, "custom map patch failed"):
                self._run_fake_pipeline(tmp, discovery=0, trainer=0, hub=0, custom=1)

    def test_pipeline_blocks_when_map_plan_fails(self):
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(RuntimeError, "map plan failed"):
                self._run_fake_pipeline(tmp, hub=1)

    def test_pipeline_blocks_when_custom_map_command_omits_output(self):
        with tempfile.TemporaryDirectory() as tmp:
            builder = load_builder()
            root, dpe = make_fake_workspace(tmp)
            (root / "BPRE0.gba").write_bytes(b"pristine")
            ...
            def fake_build(label, cwd):
                payload = b"dpe-expanded" if label == "DPE" else b"dpe-plus-cfru"
                (cwd / "test.gba").write_bytes(payload)

            with self.assertRaisesRegex(RuntimeError, "did not produce an output"):
                builder.run_pipeline(
                    cfru_root=root,
                    dpe_root=dpe,
                    output_path=output,
                    ...
                    sync_dpe=lambda _p: None,
                    verify_dpe_symbols=lambda _p: None,
                    validate_starter_runtime=lambda _p: None,
                    apply_preview_patch=lambda source, destination: destination.write_bytes(
                        source.read_bytes() + b"-preview"
                    ),
                    audit_opening=lambda _p: None,
                    discover_port_link=lambda _p: 1,
                    apply_port_link_trainer=lambda _s, _d: 1,
                    prepare_delivery_hub_map=lambda _p: 0,
                    apply_custom_maps=lambda _s, _d: 0,
                )
            self.assertFalse(output.exists(), "blocked build left an incomplete candidate ROM")
