...
class TestCustomMapsPatch(unittest.TestCase):
    def fixture(self):
        ...
        # Castello reserved source slot: 1x1 prototype with a live connection.
        cl=bytearray(b"\x00"*0x1C)
        cl[0:4]=(1).to_bytes(4,"little",signed=True)
        cl[4:8]=(1).to_bytes(4,"little",signed=True)
        cl[8:12]=ptr(0x4800);cl[12:16]=ptr(0x4820)
        cl[16:20]=ptr(0x4840);cl[20:24]=ptr(0x4860)
        cl[24]=2;cl[25]=2
        rom[0x2C00:0x2C1C]=cl

        ch=bytearray(b"\x00"*0x1C)
        ch[0:4]=ptr(0x2C00);ch[4:8]=ptr(0x2D00);ch[8:12]=ptr(0x2E00);ch[12:16]=ptr(0x2F00)
        ch[16:18]=castello.MUSIC_SEVII_ROUTE.to_bytes(2,"little")
        ch[18:20]=(0x4444).to_bytes(2,"little")
        ch[20]=castello.MAPSEC_SEVII_ISLE_6;ch[21]=0;ch[22]=castello.WEATHER_SUNNY;ch[23]=castello.MAP_TYPE_ROUTE
        ...
        # Deploy Room reserved source slot.
        rl=bytearray(b"\x00"*0x1C)
        rl[0:4]=(11).to_bytes(4,"little",signed=True)
        rl[4:8]=(9).to_bytes(4,"little",signed=True)
        rl[8:12]=ptr(0x4900);rl[12:16]=ptr(0x4920)
        rl[16:20]=ptr(0x4940);rl[20:24]=ptr(0x4960)
        rl[24]=2;rl[25]=2
        rom[0x3400:0x341C]=rl

        rh=bytearray(b"\x00"*0x1C)
        rh[0:4]=ptr(0x3400);rh[4:8]=ptr(0x3500);rh[8:12]=ptr(0x3600)
        rh[12:16]=(0).to_bytes(4,"little")
        rh[16:18]=room.MUSIC_ROUTE3.to_bytes(2,"little")
        rh[18:20]=(0x6666).to_bytes(2,"little")
        rh[20]=room.MAPSEC_ROUTE6;rh[21]=0;rh[22]=room.WEATHER_NONE;rh[23]=room.MAP_TYPE_INDOOR
        rh[24]=0;rh[25]=0;rh[26]=0;rh[27]=room.BATTLE_SCENE_NORMAL
        rom[0x1500:0x151C]=rh
